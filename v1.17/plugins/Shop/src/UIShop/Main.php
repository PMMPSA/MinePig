...
class Main extends PluginBase
{
    ...
    public function Category($cfg, Player $player, Config $msg): void
    {
        ...
        $form->setTitle("§l§e•§c Shop §e•");
        $form->setContent("§l§c↣ §aTiền của bạn: §e".$money);
        if ($this->getConfig()->get("Category_ExitButton") == true) {
            $form->addButton("§l§e• §cThoát §e•", 2);
        }
        foreach ($cfg as $cate => $category) {
            if ($category == self::SHOP_VERSION) {
                ...
                if (substr($list[1], 0, 4) == "http") {
                    $form->addButton("§l§e• §a".$list[0]." §e•", 1, $list[2]);
                } else {
                    $form->addButton("§l§e• §a".$list[0]." §e•", 0, $list[1]);
                }
            }
        }
            ...
            foreach ($items["Items"] as $cate => $item) {
                ...
                if ($list[0] == "cmd") {
                    ...
                    if (substr($list[6], 0, 4) == "http") {
                        ...
                        $form->addButton("§l§e• §a".$name . " §e•" , 1, $list[6] . ":" . $list[7]);
                    } else {
                        #$form->addButton($name . " " . $list[3] . $msg->getNested("Messages.Each"), 0, $list[6]);
                        $form->addButton("§l§e• §a".$name . " §e•" , 0, $list[6]);
                    }
                }
            }
            $form->sendToPlayer($player);
                    ...
                    if ($data[1] == false) {
                        if ($money >= $list[3] * $data1) {
                            ...
                            if ($list[5] != "Default") {
                                ...
                                $xu = 0;
       for($i = 0;$i<= $data1 - 1;$i++){
         if (!$player->getInventory()->canAddItem(Item::get(0, 0, 1))){
            $player->sendMessage("§l§aBạn đã mua §f $count §e$name §avới giá §f$xu §athành công");
            return;
        }
        $count = $count + 1;
        $xu = $xu + $list[3];
                                $item->addItem(Item::get((int)$list[0], (int)$list[1], 1));
                EconomyAPI::getInstance()->reduceMoney($player, $list[3]);                              
       }
                            }
        ...
        $money = $this->getServer()->getPluginManager()->getPlugin("EconomyAPI")->myMoney($player);
        $form->setTitle("§l§e•§c Xác Nhận Mua §a$name §e•");
        $form->addLabel("§l§c↣ §aTiền của bạn: §e$money\n\n".$message);
        if ($this->getConfig()->getNested("Types.Toggle") == true) {
        ...
        if ($this->getConfig()->getNested("Types.Slider") == true) {
            $form->addSlider("§l§e• §cSố Lượng", $this->getConfig()->getNested("Types.Slider_Minimum"), $this->getConfig()->getNested("Types.Slider_Maximum"));
        }
        if ($this->getConfig()->getNested("Types.StepSlider") == true) {
