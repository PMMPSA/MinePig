			...
			if(!($sender instanceof Player)){
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(!$sender->isSurvival()){
						$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Hãy chuyển sang chế độ sinh tồn!");
						return false;
					}
...
$this->uplevel($sender);
        ...
        $uplevel = $level * 10000;
        $form->setTitle("§l§e•§a Info §e•");
          $form->setContent("§l§e•§a Cấp§f: §e".$level."\n§e•§a Kinh Nghiệm§f: §e".$kn."§f/§c".$uplevel."\n§l§c↣§a Bạn nhận thêm được ".$level." phần trăm khi sell");
   ...
   if($level < 100){
						...
						$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§a Bán được §f".$xu. " §bXu §bvà được §f".$lx." §bXu§b từ sell level");
   }  
   if($level == 100) {
						$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§a Bạn bán được §f".$xu. " §bXu §bvà được §f".$xl." §bXu§b từ sell level");
   }
   EconomyAPI::getInstance()->addMoney($sender, $xu + $lx);
   $this->uplevel($sender);
					}elseif(isset($args[0]) && strtolower($args[0]) == "about"){
						$sender->sendMessage(TF::RED . TF::BOLD . "(!) " . TF::RESET . TF::GRAY . "Server sử dụng plugin Sell Hand, bởi Muqsit Rayyan và sửa bởi georgianYT Và Sửa Lại Lần 2 bởi PIG");
					}else{
						$sender->sendMessage("§l§e•§a Sell Ore §e•");
						$sender->sendMessage("§l§c↣§a Sử dụng: §b/sell all §ađể bán tất cả quặng trong túi đồ");
						$sender->sendMessage("§l§c↣§a Sử dụng: §b/sell info §ađể xem thông tin của bạn");
						return true;
					}
				}else{
   ...
   $kn = $this->kn->get($sender->getName());
       ...
       $this->level->save();
      $level = $this->level->get($sender->getName());
							$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§a Chúc mừng Sell Level của bạn đã lên:§e ".$level);
							$this->uplevel($sender);
       }
	}
}
