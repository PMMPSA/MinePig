...
class Quests
{
	...
	*/
	public function validatePlayerQuest(Player $player, $quest) : bool
	{
		if($this->hasQuest($player) == false)
		{
			...
			$player->sendMessage("§l§a[§e Mine §dPIG §a]§c§l Đã xảy ra lỗi, nhiệm vụ có thể đã bị xóa trong quá trình");
			return false;
		}
	...
	*/	
	public function Completed(Player $player) : bool
	{
			...
			if( $this->hasQuest($player)){
				...
				if($hand->getId() == $item->getId()){
					if($hand->getCount() >= $item->getCount()){
						...
						$player->addTitle("§l§aHoàn Thành" , "§6". $this->getQuestTitle($quest));
		...
		$player->sendMessage("§l§a[§e Mine §dPIG §a]§c Vui lòng chỉnh chế độ sinh tồn.");
		return false;
	}
	...
	*/
	public function sendQuestInfo(Player $player, string $quest){
		...
		$form->setContent("". $this->getQuestTitle($quest). "\n". $this->getQuestInfo($quest));
