...
class KitInfo extends Window {
	...
	public function process(): void{
		...
		$this->data = [
			...
			"title"   => $title,
			"content" => $info,
			"button1" => "§l§e•§a Đồng Ý §e•",
			"button2" => "§l§e•§c Thoát §e•",
		];
	}
	...
	private function select($index){
		...
		switch($index){
			case "true\n":
				...
				if(isset($kits->data["coin"])){
					$coin = $kits->data["coin"];
					if(CoinAPI::getInstance()->myCoin($this->player) < $coin){
						$error = $this->pl->language->getTranslation("cant-afford", $name, $coin);
						$this->pl->formData[strtolower($this->player->getName())]["error"] = $error;
						$this->navigate(Handler::KIT_ERROR, $this->player, $windowHandler);
						break;
					}
