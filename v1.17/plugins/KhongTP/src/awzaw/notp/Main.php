...
class Main extends PluginBase implements Listener {
    ...
    public function onCommand(CommandSender $issuer, Command $cmd, string $label, array $args) : bool{
        if ((strtolower($cmd->getName()) == "khongtp") && !(isset($args[0])) && ($issuer instanceof Player) && ($issuer->hasPermission("khongtp.command") || $issuer->hasPermission("khongtp.command.self"))) {
            ...
            if (isset($this->enabled[strtolower($issuer->getName())])) {
                $issuer->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§a Bạn đã bật chế độ không tp!");
            } else {
                $issuer->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Bạn đã tắt chế độ không tp!");
            }
            return true;
        } else {
            return false;
        ...
        if (strtolower(substr($message, 0, 3)) == "/tp" || strtolower(substr($message, 0, 5)) == "/call" || strtolower(substr($message, 0, 7)) == "/tphere" || strtolower(substr($message, 0, 4)) == "/tpa") {
            ...
            foreach ($this->enabled as $notpuser) {
                ...
                if ((strpos(strtolower($notpuser), strtolower($args[1])) !== false) && (strtolower($notpuser) !== strtolower($sender->getName()))) {
                    $sender->sendMessage(TextFormat::RED . "§l§c•§e Người chơi này đã bật không tp!");
