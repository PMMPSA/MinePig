            ...
            if($damager instanceof Player){
                ...
                if($name !== $this->player){
                    if(!$damager->hasPermission("minion.open.others")){
                        $damager->sendMessage("§l§e•§c Đây không phải osin của bạn");
                        return;
                    }
                    ...
                    if($this->check == null){
                        if($owner->getInventory()->canAddItem($spawn)){
                           Main::get()->flagEntity($owner, $this);  
                            ...
                            $owner->sendMessage("§l§e•§c Túi của bạn đã đầy, osin không thể quay lại vào túi");
                            $owner->sendMessage("§l§e•§c Hãy cất bớt vật phẩm"); 
                            $this->check = true;
                        }                    
                    }else{
                        if($owner->getInventory()->canAddItem($spawn)){
