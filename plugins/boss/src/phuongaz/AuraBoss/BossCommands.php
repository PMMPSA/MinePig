...
Class BossCommands extends Command{
	...
	public function __construct(Boss $main){
		parent::__construct("boss", "Boss", "/boss");
		$this->main = $main;
	}
	...
	public function execute(CommandSender $sender, string $label, array $args):bool {
		...
		if(isset($args[0]) and $args[0] == 'clear-all'){
			$sender->sendMessage("§l§e•§a Tất cả boss đã xoá");
	...
	public function openForm(CommandSender $sender){
		...
		$form->setTitle("§l§f•§9 Boss §f•");
		$form->addButton("§l§c•§9 Hồi Sinh Boss §c•",0,"textures/other/spawn");
		$form->addButton("§l§c•§9 Tạo Boss §c•",0,"textures/other/create");
	...
	public function openSpawnForm(Player $player) {
		$form = new SimpleForm(function(Player $player, $data){
			if(is_null($data)) return;
			$player->sendMessage("§l§e•§a Boss đã được hồi sinh");
			$this->main->spawnBoss($data);
		});

		$form->setTitle("§l§f•§9 Hồi Sinh Boss §f•");
		foreach(array_keys($this->main->getAllBoss()) as $id){
			$name = $this->main->config->get($id)["name"];
			$form->addButton("§l§c•§9 Hồi Sinh §c•\n§l".$name);
		}
	...
	public function openSettingForm($player){
		...
		$form->setTitle("§l§f•§9 Tạo Boss §f•");
		$form->addInput("§l§c•§e Type:");
		$form->addInput("§l§c•§e Name:");
		$form->addInput("§l§c•§e Health:");
		$form->addInput("§l§c•§e Damage:");
		$form->addInput("§l§c•§e Speed:");
		$form->addInput("§l§c•§e Size:");
		$form->sendToPlayer($player);	
	}
}