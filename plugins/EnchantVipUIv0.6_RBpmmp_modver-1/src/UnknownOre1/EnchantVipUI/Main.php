...
use UnknownOre1\EnchantVipUI\forms\{
    CustomForm,
    SimpleForm
};
...
class Main extends PluginBase{
    ...
    public function onEnable(): void{
        if (is_null($this->getServer()->getPluginManager()->getPlugin("CoinAPI"))) {
            $this->getLogger()->error("Neu ban muon dung EnchantVipUI ban can phai tai CoinAPI.");
        ...
        $this->getServer()->getCommandMap()->register("buyecvip", new Commands\ShopCommand($this));
    ...
    public function UpdateConfig(): void{
        if(is_null($this->shop->getNested('version'))){
            ...
            $this->shop->reload();
            $this->getLogger()->notice("Config cua plugin da duoc cap nhat!");
            return;
        }
    ...
    */
    public function buyForm(Player $player,int $id): void{
        ...
        $form = new CustomForm(function (Player $player, $data = null) use ($array, $id){
            ...
            if(CoinAPI::getInstance()->myCoin($player) > $c = $array[$id]['price'] * $data[1]){
                CoinAPI::getInstance()->reduceCoin($player, $c);
