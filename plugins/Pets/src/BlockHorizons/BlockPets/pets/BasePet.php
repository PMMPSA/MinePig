...
abstract class BasePet extends Creature implements Rideable {
	...
	public function levelUp(int $amount = 1, bool $silent = false): bool {
		...
		if(!$silent && $this->getPetOwner() !== null) {
			$this->getPetOwner()->addTitle((TextFormat::GREEN . "§l§6♦§a Lên Cấp §6♦"), (TextFormat::AQUA . "§l§f(§e Pet§f:§b " . $this->getPetName() . TextFormat::RESET . TextFormat::AQUA . " §l§aCấp§f: §e" . $ev->getTo() . "§f )"));
		}
		return true;
	}
