...
class Kit {
	...
	public function __construct(Main $pl, array $data, string $name){
		...
		if(isset($this->data["coin"]) and $this->data["coin"] != 0){
			$this->cost = (int)$this->data["coin"];
		}
	...
	public function add(Player $player){
		...
		if(isset($this->data["coin"])){
			$event = new KitPurchaseEvent($player, $this, $this->data["coin"]);
			$event->call();
			...
			CoinAPI::getInstance()->reduceCoin($player, $event->getPrice());
		}
		skipPurchase:
