      ...
      $inv = $player->getInventory();
        if (($this->getTrade($id)['support']['money']) == "true")
		{
			...
			if((EconomyAPI::getInstance()->myMoney($player)) >= $money){
				...
				$player->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§a Bạn đã mua vật phẩm này thành công!");
				$inv->addItem($new);
			}else{
			$player->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Bạn không đủ tiền để mua vật phẩm này");
			}
		}
