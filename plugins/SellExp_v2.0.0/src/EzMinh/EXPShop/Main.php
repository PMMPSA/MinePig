...
Class Main extends PluginBase implements Listener{
	...
	public function onEnable():void{
		...
		$this->getLogger()->info("§l§a> §bPlugin Code Bye Ezminh, LetTIHL");
    ...
    public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args): bool
    {
        ...
        $money = $player_exp * 1000;
					...
					$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§e Bán exp thành công, nhận được§a " . $money . "§eXu");
		}elseif ($cmd->getName() == "autosellexp"){
            $auto_aell_exp = $this->auto->get($sender->getName());
		    if($auto_aell_exp == "no"){
                $this->auto->set($sender->getName(), "yes");
			    $sender->sendMessage("  §l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§e Đã Bật Auto Sell Exp");
		    }elseif($auto_aell_exp == "yes"){
                $this->auto->set($sender->getName(), "no");
				$sender->sendMessage("§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§e Đã tắt Auto Sell Exp");
		  }
		return true;
       }
     return true;
